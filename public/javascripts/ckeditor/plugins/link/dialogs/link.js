...
//				partIds = [ 'urlOptions', 'anchorOptions', 'emailOptions', 'wikiOptions' ],
	...
	var parseLink = function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				javascriptMatch, emailMatch, anchorMatch, urlMatch, wikiMatch,
				retval = {};
...
//					[ linkLang.toWiki, 'wiki' ]
				...
				children: [
					...
					children: [
  					...
  					setup: function( data ) {
  						if ( data.wiki )
  							this.setValue( data.wiki );
  					},
  					commit: function( data ) {
   						data.wiki = this.getValue();
  					}
					}
					],
					setup: function( data ) {
		...
		onOk: function() {
			...
			switch ( data.type || 'url' ) {
				case 'wiki':
					var name = data.wiki;
  					attributes[ 'data-cke-saved-href' ] = '#' + name;
  					attributes[ 'class' ] = 'wiki-page';
  					changedLinkName = name;
					break;
				case 'url':
			...
			if ( !this._.selectedElement ) {
				...
				if (changedLinkName != "") element.setHtml(changedLinkName);
				else element.removeClass('wiki-page');
